...
#--------------------------------------------------
# - Creating a Linux VMSS
#--------------------------------------------------
resource "azurerm_linux_virtual_machine_scale_set" "this" {
  name                                              = module.azu-product-tf-resourcenames.names.azurerm_linux_virtual_machine_scale_set
  ...
  network_interface {
    name                          = "${module.azu-product-tf-resourcenames.names.azurerm_network_interface}-nic"
    primary                       = true
    ...
    dynamic "ip_configuration" {
      ...
      content {
        name                                         = "${module.azu-product-tf-resourcenames.names.azurerm_network_interface}_ip_configuration_${index(var.network_interface.ip_configurations, ip_configuration.value)}"
  ...
  dynamic "identity" {
    for_each = local.identityType == null ? [] : [1]
    content {
  ...
  dynamic "plan" {
    for_each = var.image_marketplace == false ? [] : [1]
    content {
      name      = var.plan.name
...
#---------------------------------------------------------------
# - Add Network Watcher extension in Virtual machine scale set
#--------------------------------------------------------------

...
module "BBL_test_vnetp" {
  ...
  providers = {
    azurerm.spoke = azurerm.spokecon
    azurerm.hub   = azurerm.hubcon
  }

  # Local use
      source = "../../Platform-Level/networking/Platform-Product/terraform-azurerm-virtualnetwork-peering/v1"

  # virtualnetwork-peering settings
  spoke_vnet_id = module.virtual_network_Spoke.id
  hub_vnet_id   = module.virtual_network_hub.id

  depends_on = [ module.resource_group_hub, module.resource_group_spoke, module.virtual_network_hub, module.virtual_network_Spoke ]
}